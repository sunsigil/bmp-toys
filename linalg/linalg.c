...
mat_t homogenize(mat_t row)
{
	return mat_scale(row, 1.0 / row.data[3]);
}
...
mat_t lookat(mat_t eye, mat_t target, mat_t up)
{
	...
	double x = mat_dot(u, eye);
	double y = mat_dot(v, eye);
	double z = mat_dot(w, eye);

	mat_t A = mat_id(4);
	A = mat_blit(A, 0, 0, u);
	A = mat_blit(A, 1, 0, v);
	A = mat_blit(A, 2, 0, w);
	A = mat_blit(A, 0, 3, mat_col(3, x, y, z));
...
mat_t perspective(double aspect, double fovy, double near, double far)
{
	...
	double z_norm = near / (near - far);
	double z_off = -far * z_norm;
